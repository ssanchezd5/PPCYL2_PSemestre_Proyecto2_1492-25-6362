...
def admin_panel(request):
    if request.session.get('rol') != 'administrador': return redirect('/')
    try:
        r = requests.get('http://127.0.0.1:5000/obtener_usuarios')
        usuarios = r.json() if r.status_code == 200 else []
    except:
        usuarios = []
    return render(request, 'admin_panel.html', {'usuarios_sistema': usuarios})


# 3. Acciones de Administrador
def crear_usuario_view(request):
    if request.method == 'POST':
        d = {'nombre': request.POST.get('nuevo_nom'), 'pass': request.POST.get('nuevo_pass'),
...
def eliminar_usuario_view(request):
    if request.method == 'POST':
        requests.post('http://127.0.0.1:5000/eliminar_usuario', json={'nombre': request.POST.get('nombre_eliminar')})
    return redirect('/admin_panel/')


# 4. Panel de Maestro (Detección de Materia y Carga de XML)
def subir_xml(request):
    if request.session.get('rol') != 'maestro': return redirect('/')
    u = request.session.get('usuario', '')

    # DETECCIÓN DE MATERIA (Incluye Cálculo ahora)
    if "mate" in u.lower():
    ...
    if request.method == 'POST':
        ...
        # Guardado de Horario (Corregido para Cálculo)
    ...
    # PROCESO DE CARGA DE TABLA: Filtra alumnos e ignora tildes
    try:
        ...
        for x in todos:
            ...
            for m_db, calif in notas.items():
                # Comparamos quitando tildes para evitar errores
                m_db_clean = m_db.lower().replace('á', 'a').replace('é', 'e').replace('í', 'i').replace('ó',
                                                                                                        'o').replace(
                    'ú', 'u')
                m_maestro_clean = materia_maestro.lower().replace('á', 'a').replace('é', 'e').replace('í', 'i').replace(
                    'ó', 'o').replace('ú', 'u')

                if m_maestro_clean in m_db_clean:
...
def alumno_view(request):
    ...
    return render(request, 'alumno.html', {'usuario': u, 'notas': notas, 'horarios': horarios})
